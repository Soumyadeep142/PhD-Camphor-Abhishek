...
damping = 0.2
molarity = 1.0
alignment_strength = molarity * 0.3
wall_align = 0.8
alignment_range = 2 * circum_radius

R = 100
cx, cy = L / 2, L / 2

os.makedirs(f"outputs/Rep_{edge_repulsion_strength}", exist_ok=True)
...
def compute_edge_edge_repulsion(poly_i, poly_j):
    F = np.zeros(2)
    coords_i = list(poly_i.exterior.coords)
    coords_j = list(poly_j.exterior.coords)
    edges_i = list(zip(coords_i, coords_i[1:]))
    edges_j = list(zip(coords_j, coords_j[1:]))

    for p1, p2 in edges_i:
        edge_i = LineString([p1, p2])
        midpoint_i = np.array([(p1[0] + p2[0]) / 2, (p1[1] + p2[1]) / 2])

        for q1, q2 in edges_j:
            edge_j = LineString([q1, q2])
            if edge_i.intersects(edge_j):
                continue
            midpoint_j = np.array([(q1[0] + q2[0]) / 2, (q1[1] + q2[1]) / 2])
            dx = midpoint_j[0] - midpoint_i[0]
            dy = midpoint_j[1] - midpoint_i[1]
            r = np.sqrt(dx**2 + dy**2)
            if r > 3 * interaction_sigma:
                continue
            force_mag = edge_repulsion_strength * np.exp(-(r / edge_sigma)**2)
            fx = -force_mag * dx / r
            fy = -force_mag * dy / r
            F += np.array([fx, fy])
    return F
    ...
    for t in range(n_frames):
        ...
        for i, (x, y, theta) in enumerate(particles):
            ...
            for j in range(len(particles)):
                if i != j:
                    F += compute_edge_edge_repulsion(triangle_shapes[i], triangle_shapes[j])
...
# Combine all particle logs
with open(f"outputs/Rep_{edge_repulsion_strength}/particle_trajectories_N_{N_particles}_{A}_rep_{edge_repulsion_strength}.csv", "w", newline='') as fout:
